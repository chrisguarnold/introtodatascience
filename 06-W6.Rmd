...
For example, [drinking beer](https://youtu.be/dX5QNZtQXX4) and being thirsty is a negative relationship: the more beer you drink, the less thirsty you are. However, the relationship between a headache the next day and drinking beer is a positive one: the more beer you drink, the larger the headache the next morning.
...
kable(list(table.breakfast.summer.18, table.breakfast.summer.19, table.breakfast.summer.21),
      caption = "Cross Tabulating the variables 'What You Drink in the Morning' and 'How Much You Like Summer' for the Cohort in 2018, 2019 and 2022 (from Left to Right).",
      booktabs = TRUE,
      valign ='t')
```
...
* Cohort 2022: `r round(CramerV(dat$breakfast, dat$summer), 2)`

Here, the quick analysis tells us that there is a moderate relationship at play. It is strongest for the cohort in 2018: It looks as if it were driven by summer loving coffee drinkers vs. tea drinkers who seem to prefer other seasons. In the 2022 cohort there is the weakest relationship among the four.
...
kable(list(table.siblings.sleep.18, table.siblings.sleep.21, table.siblings.sleep),
      caption = "Tabulating the Number of Siblings with the Hours You Sleep at Night for the Cohort in 2018, 2021 and 2022 (from Left to Right).",
      booktabs = TRUE,
      valign ='t')
```
...
* Cohort 2022: `r round(GoodmanKruskalGamma(dat$siblings, dat$sleep), 2)`


Overall, the relationship turns out to be negative in all cases. Interesting! It is weakly negative for the cohort in 2019, moderately negative for the cohort 2018. For the cohort 2021, it even shows an almost strong negative relationship. Looking at the data from this cohort we can immediately see why: Single childs never sleep less than 8 hours and those with one sibling never less than 7.
...
How long is your commute to university? It turns out, it takes the men in the class on average `r commute.men` mins. The women, however, take `r commute.women` mins. This looks like a bit of a  difference.
