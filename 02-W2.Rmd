...
<!-- Next time, add video intro to setting the working directory here for windows and unix. Students struggle... -->
...
Take a look at your data. Your mean is slightly higher than your median. Why is that so? Basically, those who study more than 30h per week have a considerable influence on the mean and push the mean to the right of the median.

Compare your data with the data from the previous cohort: Here the median and the mean are closer together. Why is this so?
...
We can take this a little further and ask *How many computers did 10 student buy in 2020?*. To answer this question, we simply adapt the number of people in the unit and set $u=10$.
```{r echo=FALSE}
u <- 10
```
\begin{equation*}
...
So 10 students bought `r round((f/t)/(N/u),2)` computers in 2020.
...
```{r echo=FALSE}
f_1 <- table(computer20)[2]
f_2 <- table(computer21)[2]
```
\begin{align*}
	G &= \left(\frac{f_2 - f_1}{f_1}\right) 100 \\
	 &= \left(\frac{\text{Purchases in 2021} - \text{Purchases in 2020}}{\text{Purchases in 2020}}\right) 100 \\
	 &= \left(\frac{`r f_2 - f_1`}{`r f_1`}\right) 100 = `r round(100*(f_2 - f_1)/f_1, 2)`\%
\end{align*}

Apparently, you were purchasing less computers during 2021, more specifically, the sales had a negative growth of $G = `r round(100*(f_2 - f_1)/f_1, 2)`\%$. It could be that a number of you bought a new computer when you started your university career back in 2020.
...
Now, with a bit more of a background, we can calculate all of this week's statistics. In R, this is *really* straightforward. 

### Working with Data Frames

First, we load the data. Then we check which type it is: Apparently a data frame.
```{r}
dat <- read.csv("preparing_data/data/class_survey/lecture_survey_22.csv")
class(dat)
```
