...
axis(2, col = "white", las = 1, at = c(1,2,3,4),lab =c("Module 2018", "Module 2019", "Module 2021", 'You'))
axis(1)
boxplot(list(dat18$studyideal, dat19$studyideal, dat21$studyideal, studyideal), horizontal = TRUE
        , col = cardiffred
        , axes = FALSE
        , boxwex = .6
        , ylim = c(0,60)
...
It seems as if you are investing a little less than the students in 2019 and 2018, but more than the lockdown students from 2021. Now when you compare you to the previous years with regards to how much you think you should be studying, your ideal number of hours is not so different and you seem to be a bit more in line with the other students.
...
The means are even further apart. The cohort in 2019 was putting in on average `r round(ttest.perweek.now.19$estimate[1], 2)` hours, while you on average study `r round(ttest.perweek.now.19$estimate[2], 2)` hours. Can we still assume that the mean in both cohorts is the same in the light of the data we collected from the students in the two methods classes?

The p-value is reported as `r round(ttest.perweek.now.19$p.value, 2)`. If both of the samples were drawn from cohorts with the same mean, we would have a `r round((ttest.perweek.now.19$p.value*100), 2)`% probability that we observe the data in our two samples. This is still quite unlikely and below the general accepted the threshold of $\alpha$ = 0.05. So what do we do now? Well, we refute the null hypothesis $H_0$. Instead, we will trust in the alternative hypothesis $H_A$. Your cohort is on average studying less than the cohort in 2019.

And lastly, let us compare you to the cohort before you that was completely in lockdown in their second year.


```{r echo=FALSE}
ttest.perweek.now.21 <- t.test(dat21$studyperweek, studyperweek)
ttest.perweek.now.21
```


They were studying on average `r round(ttest.perweek.now.21$estimate[1], 2)` hours. In comparison to your `r round(ttest.perweek.now.21$estimate[2], 2)` hours, do both larger cohorts study the same?

The t-test yields a p-value of `r round(ttest.perweek.now.21$p.value, 2)`. If both of the samples were drawn from cohorts with the same mean, we would have a `r round((ttest.perweek.now.21$p.value*100), 2)`% probability that we observe the data in our two samples. Again. We refute the 0-hypothesis. 
